    ...
    <LinearGradient colors={[colors.cream, colors.sky + '22']} style={styles.container}>
      ...
      <View style={styles.grid}>
        {round.options.map((option, i) => {
          ...
          return (
            <Animated.View key={option} style={isWrong ? { transform: [{ translateX: shake.interpolate({ inputRange: [-1, 1], outputRange: [-10, 10] }) }] } : undefined}>
              <BouncyButton style={[styles.option, { backgroundColor: palette.bg }]} onPress={() => handleAnswer(option)} accessibilityLabel={option}>
...
const styles = StyleSheet.create({
  ...
  prompt: { fontFamily: fonts.bodyBold, fontSize: 16, color: colors.ink, marginTop: spacing.sm, marginBottom: spacing.md, textAlign: 'center' },
  ...
  optionText: { fontFamily: fonts.displayBold, fontSize: 18, color: colors.white },
