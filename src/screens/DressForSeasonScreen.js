...
export default function DressForSeasonScreen({ route, navigation }) {
  ...
  useEffect(() => {
    speak(`It's ${round.season.label}! What should you wear?`, { rate: 0.95, pitch: 1.15 });
  }, [index]);
  ...
  return (
    <LinearGradient colors={[colors.cream, colors.sky + '22']} style={styles.container}>
      ...
      <View style={styles.seasonBox}>
        <Text style={styles.seasonEmoji}>{round.season.emoji}</Text>
        <Text style={styles.seasonLabel}>{round.season.label}</Text>
      </View>
      ...
      {showCelebration && (
        <View style={styles.celebrationOverlay} pointerEvents="none">
          ...
          <Text style={styles.celebrationText}>Perfect choice!</Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  seasonBox: {
    marginTop: spacing.md, backgroundColor: colors.white, borderRadius: radius.lg, padding: spacing.md, alignItems: 'center',
    shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, elevation: 3,
  },
  seasonEmoji: { fontSize: 60 },
  seasonLabel: { fontFamily: fonts.displayBold, fontSize: 18, color: colors.ink, marginTop: 4 },
